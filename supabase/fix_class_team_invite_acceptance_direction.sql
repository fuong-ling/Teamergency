-- Teamergency: fix class-team invite acceptance direction and request state.
--
-- MANUAL SUPABASE EXECUTION ONLY. Do not run from the application.
-- This preserves the existing RPC signature and return contract.
--
-- The invite's team_request_id is the authoritative target. The invitee is
-- inserted into that request and never into a request owned by the invitee.
...
begin
  ...
  if p_response = 'accepted' then
    -- The inviter owns the explicit target request. Never substitute an
    -- active request owned by the invitee.
    select tr.*
    into target_request
    ...
    ) and current_size >= total_size then
      raise exception 'Team is full or no longer open.';
    end if;

    -- A profile cannot remain in another active team for the same class and
    -- session while accepting this invitation.
    if exists (
      select 1
      from public.class_team_memberships ctm
      ...
      where ctm.profile_id = p_profile_id
        and ctm.team_request_id <> invite_row.team_request_id
        and other_request.class_id = invite_row.class_id
    ...
    -- Resolve only empty requests owned by the invitee. If another active
    -- teammate exists, block instead of orphaning that team.
    for old_request in
      select tr.id
      from public.team_requests tr
      where tr.profile_id = p_profile_id
        and tr.id <> invite_row.team_request_id
        and tr.class_id = invite_row.class_id
